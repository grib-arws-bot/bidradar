...
def test_run_pending_embeddings_does_not_batch_attachment_variant():
    ...
    묶었더니 실전 배포에서 오히려 느려졌다(건당 8~16초 -> 24~32초). sentence-transformers가
    배치 안 최장 시퀀스에 맞춰 짧은 텍스트도 패딩해서, 길이가 들쭉날쭉한 attachment는
    크게 묶을수록 연산 낭비가 커진다고 보고 더 작은 배치(4)로 재실험했으나 그것도
    건당 30~32초로 여전히 느렸다(두 차례 측정 일관). attachment는 배치화 자체가 이
    서버·이 텍스트 특성에 안 맞는다고 결론짓고 1건씩(무배치)으로 확정 — 회귀 방지 테스트."""
    with mock.patch(
