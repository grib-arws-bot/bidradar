...
def patch_email_schedule(
    ...
    """보고서 메일 자동발송 요일·시간(2026-09-14) — app/scheduler.py의 run_due_customer_emails가
    이 값을 그대로 읽어 실제 발송을 실행한다(app/services/customer_management.py의
    set_email_schedule/validate_email_schedule 설명 참고)."""
    with engine.begin() as conn:
        try:
            found = set_email_schedule(conn, customer_id, payload.days, payload.time)
        ...
        audit.record(
            ...
            detail={"days": payload.days, "time": payload.time},
        )
    return {"id": customer_id, "days": payload.days, "time": payload.time}
