...
from app.models import analysis, analysis_doc, analysis_eligibility, analysis_requirement
from app.security.url_guard import fetch
from app.services.analysis.eligibility_extract import (
    _ELIGIBILITY_PROMPT_NOTE,
    _ELIGIBILITY_STRUCTURED_SCHEMA,
    valid_eligibility_axes,
)
from app.services.analysis.structure_query import rescan_interest_topics
...
- project_period: 사업(연구개발)기간. **최대한 짧게** — 과제마다 기간이 다르면 대표값(가장
  긴 것) + "과제별 상이"로. 예: "43개월(당해 9개월), 과제별 상이" 또는 과제가 하나면
...
- submission: 제안제출 {deadline(제출기한), method(제출방법·사이트), documents(제출서류
  목록 — 문서마다 한 항목씩 배열로, 표를 옮기지 말고 서류명만 간결하게)}.
            ...
            "properties": {
                ...
                "eligibility": {
                    ...
                    "properties": {
                        ...
                        "structured": _ELIGIBILITY_STRUCTURED_SCHEMA,
                    },
                    "required": [
                        "consortium", "lead_org", "participant_org", "demand_org", "company_size", "special_notes",
                        "structured",
                    ],
                },
                "submission": {
...
def run_structuring(conn: Connection, analysis_id: int, *, model: str = "claude-haiku-4-5-20251001") -> dict:
    ...
    eligibility_axes = valid_eligibility_axes((summary or {}).get("eligibility", {}).get("structured"))
    if eligibility_axes:
        conn.execute(insert(analysis_eligibility).values(analysis_id=analysis_id, **eligibility_axes))

    topics_added = rescan_interest_topics(conn, row.notice_id, summary) if summary else 0
    ...
    eligibility_axis_value_keys = (
        "company_size_allowed_tiers", "research_institute_required", "venture_cert_required",
        "industry_codes_required", "certifications_required",
    )
    eligibility_axes_saved = (
        sum(1 for k in eligibility_axis_value_keys if eligibility_axes.get(k) is not None) if eligibility_axes else 0
    )
    ...
    return {
        ...
        "eligibility_axes_saved": eligibility_axes_saved,
    }
