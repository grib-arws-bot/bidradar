...
`advisory/` 는 시스템 아키텍트(Cowork 세션)가 쓰는 폴더다.
**Claude Code는 읽기만 하고 절대 쓰지 않는다.**
여기 있는 문서는 제안이며, 승인 후 Claude Code가 docs/·코드에 반영한다.
반영 후 `docs/의사결정_로그.md` 등재는 Claude Code만 한다 —
양쪽이 같은 파일을 쓰지 않는다(2026-09-01 로그 유실 사고 재발 방지).
...
```
stg  = 개발 PC (self-hosted runner) — 빌드 + 테스트 + 로컬 기동(자동 응답 확인)까지만. 사람이 스테이징 URL을
       직접 열어보는 육안 확인은 기대하지 않는다(2026-09-15 — 사내 도구라 가용성이 크리티컬하지
       않고, 실제로도 수동 방문 없이 배포 파이프라인의 자동 게이트로만 쓰이고 있었음이 확인됨)
